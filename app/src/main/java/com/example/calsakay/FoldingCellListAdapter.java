...
public class FoldingCellListAdapter extends ArrayAdapter<Passenger> {
    ...
    private void openPassengerAcceptedFragment(Passenger passenger){
        // TODO: AYUSIN ITONG TRANSITION FROM PICKUP BUTTON TO PASSENGER ACCEPTED FRAGMENT
        Bundle bundle = new Bundle();
        bundle.putParcelable("passenger", (Parcelable) passenger);
        this.fgFindPassengers.setArguments(bundle);
        ((FragmentActivity) currentContext).getSupportFragmentManager().beginTransaction()
                .replace(R.id.flDashboard, fgFindPassengers)
                .commit();
    }
